        ...
        private void DoTicks(int ticks)
        {
            if (!this.Ruined)
            {
                ...
                if (this.ruinedPercent >= 1f)
                {
                    ...
                    Messages.Message("GR_DestroyedByTemp".Translate(parent.LabelCap), new TargetInfo(this.parent.Position, this.parent.Map, false), MessageTypeDefOf.NegativeEvent);
                    this.parent.Destroy();
